...
pub fn nodes_with_expansion(lines: &Vec<Vec<char>>, expansion: i64) -> Vec<Galaxy> {
    ...
    let i_expand: Vec<i64> = lines
        .iter()
        .enumerate()
        .filter_map(|(no, line)| if !line.contains(&'#') { Some(no as i64) } else { None })
        .collect();
    ...
    let j_expand: Vec<i64> = (0..lines.len() - 1)
        .map(|j| {
            ...
            if col { Some(j as i64) } else { None }
        })
    ...
    let nodes: Vec<Galaxy> = lines
        ...
        .map(|(i, line)| {
            line
                ...
                .filter_map(|(j, c)| {
                    if *c == '#' {
                        let gali = i as i64 + i_expand.iter().filter(|&&x| x < i as i64).count() as i64 * (expansion - 1);
                        let galj = j as i64 + j_expand.iter().filter(|&&x| x < j as i64).count() as i64 * (expansion - 1);
                        Some(Galaxy { i: gali as i64, j: galj as i64 })
                    } else {
                        None
                    }
                })
...
mod tests {
    ...
    fn day11_part1() {
        ...
        let dist_sum: i64 = nodes
            ...
            .map(|pair| {
                (pair[0].i - pair[1].i).abs() + (pair[0].j - pair[1].j).abs()
            }).sum();
