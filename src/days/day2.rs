...
mod tests {
    ...
    fn day2_part1() {
        ...
        let possible_games_id_total: u32 = lines.iter().map(|line| {
            ...
            let game_possible = turns.iter().map(|turn| {
                ...
                if red_cubes <= MAX_RED_CUBES && green_cubes <= MAX_GREEN_CUBES && blue_cubes <= MAX_BLUE_CUBES {
                    true
                } else {
                    false
                }
    ...
    fn day2_part2() {
        ...
        let total_powers: u32 = lines.iter().map(|line| {
            // First get the index of the game.
            let segments = line.split(": ").collect::<Vec<&str>>();
